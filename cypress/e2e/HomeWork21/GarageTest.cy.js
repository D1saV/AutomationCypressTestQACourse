describe('Garage tests', () => {
  ...
  it('should allow valid input and enable the Add button', () => {
    ...
    cy.get('#addCarMileage').type('25000').type('{uparrow}');
    cy.get('.modal-content')
      .contains('button', 'Add')
  ...
  it('should display all elements for a new car in the garage', () => {
    ...
    cy.get('.car-body').first().within(() => {
      ...
      cy.get('button.update-mileage-form_submit').should('be.enabled').click();
      cy.get('button.update-mileage-form_submit').should('be.disabled');
      cy.get('input[name="miles"]').should('have.value', '26300');
    });
  });

  it('should display all UI elements correctly on the edit mode', () => {
    cy.get('.car-item').first().find('.car_edit').click();
    cy.get('.modal-content').should('be.visible');
  ...
  it('should remove the car from garage', () => {
    ...
    cy.get('.modal-content').last().should('be.visible').within(() => {
      cy.get('.modal-title').contains('Remove car');
      cy.get('button.close')
        .should('have.attr', 'aria-label', 'Close')
        .find('span')
        .should('have.text', '×');
      cy.get('.modal-body').contains('Do you really want to remove BMW X5 from your account?');
      cy.get('.text-danger').contains('All related data will be lost, this action can not be undone!'
      );
      ...
      if ($body.find('.car-item').length > 0) {
        cy.get('.car-item')
          .filter((_, el) => {
            ...
            return text.includes('BMW X5') && text.includes('26300');
          })
          .should('not.exist');
      } else {
